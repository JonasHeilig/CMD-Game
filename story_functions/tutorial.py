...
def tutorial(player_data):
    ...
    time.sleep(5)
    print("You can go mine for resources, fight monsters, and level up your character.")
    time.sleep(5)
    ...
    while True:
        ...
        if action == 'yes':
            ...
            print(f"Hello {player_data['name']}! Your current coins: {player_data['coins']}. Your current Level: {player_data['character_level']}.  Your current XP: {player_data['xp']}.")
            return
    time.sleep(5)
    print("The is a simple command that helps you always. It is the 'help' command.")
    time.sleep(5)
    print("You can use the 'help' command to get a list of all available actions.")
    time.sleep(5)
    print("Now, let's start with the first action. Type 'help' to now what to do.")

    while True:
        action = input("Enter your action: ").strip().lower()
        if action == 'help':
            print("Great! You can use the 'help' command to got to now what to do.")
            time.sleep(5)
    ...
    while True:
        action = input("Enter your action: ").strip().lower()
        if action == 'exit':
            print("Great! You have learned two commands. The 'help' command and the 'exit' command.")
            time.sleep(5)
            print("When you use the 'exit' command now it will be save your game and exit it.")
            time.sleep(5)
